...
let routes = function routes() {
  ...
  router.post('/apis/augmentedvideo/v1/videos/:videoId/participant',
      ...
      bodyParser.urlencoded({'limit': '100000kb'}), (req, res) => {

    // util para depurar fotos require("fs").writeFileSync("/tmp/" + Date.now()+ '.png', req.body.img, 'base64');
    ...
    request({
      ...
      if (err || response.statusCode >= 300) {
        ...
        return res.end(JSON.stringify({err:'Error servicio Gonzalo'}));
      }
      ...
      request({
        ...
        if (err2 || response2.statusCode >= 300) {
          res.statusCode = response2.statusCode;
          return res.end(JSON.stringify({err: 'Error servicio Mongorest'}));
        }
